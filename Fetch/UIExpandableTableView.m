        ...
        if (indexPath.row == 0) {
            ...
            if ([self.myDataSource tableView:self needsToDownloadDataForExpandableSection:indexPath.section]) {
                ...
                if ([[self.showingSectionsDictionary objectForKey:key] boolValue]) {
                    ...
                    if ( openSection > -1 ) {
                        [self collapseSection:openSection animated:YES];
                    }
...
-(void) touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    NSLog([NSString stringWithFormat:@"location: %f", thebigy]);
    UITouch *touch= [[event allTouches] anyObject];
    
    CGPoint location = [touch locationInView:touch.view];
    thebigy+= location.y-ty;
    ty = location.y;
    
    
    if((currentsection*10-thebigy)*(currentsection*10-thebigy)>100){
        ...
        if(currentsection*10>thebigy){
            currentsection-=1;
        }else {
            currentsection+=1;
        }
        indexPath = [NSIndexPath indexPathForRow:0 inSection:currentsection] ;

        [self tableView:self didSelectRowAtIndexPath:indexPath];
        thebigy=10*currentsection+1;
    }
