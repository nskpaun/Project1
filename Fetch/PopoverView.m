...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withViewArray:(NSArray *)viewArray
{
    ...
    [container setBackgroundColor:[Fetch colorWithHexString:COLOR_LIGHT_GRAY]];
...
-(void)drawForSean
{
    ...
    // Build the popover path
    CGRect frame = CGRectMake(boxFrame.origin.x - 1, boxFrame.origin.y-1, boxFrame.size.width+2 , boxFrame.size.height+2);
    ...
    float radius = kBoxRadius+1; //Radius of the curvature.
    
    float cpOffset = kCPOffset+1; //Control Point Offset.  Modifies how "curved" the corners are.
    ...
    //// Gradient Declarations
    NSArray* gradientColors = [NSArray arrayWithObjects:
                               (id)[Fetch colorWithHexString:COLOR_MEDIUM_DARK_GRAY].CGColor,
                               (id)[Fetch colorWithHexString:COLOR_MEDIUM_DARK_GRAY].CGColor, nil];
    CGFloat gradientLocations[] = {0, 1};
    CGGradientRef gradient = CGGradientCreateWithColors(colorSpace, (CFArrayRef)gradientColors, gradientLocations);
